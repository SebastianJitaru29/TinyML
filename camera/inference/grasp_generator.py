...
class GraspGenerator:
    def __init__(self, saved_model_path, visualize=False):
        ...
        self.cam_depth_scale = np.array(0.0010000000474974513)

        homedir = os.path.join(os.path.expanduser('~'), "grasp-comms")
        self.grasp_request = "grasp_request.npy"
        self.grasp_available = "grasp_available.npy"
        self.grasp_pose = "grasp_pose.npy"
    ...
    def load_model(self):
        print('Loading model... ')
        self.model = torch.load(self.saved_model_path, weights_only=False, map_location=torch.device('cpu'))
    ...
    def generate(self):
        ...
        grasp_pose = np.append(target, angle[2])
